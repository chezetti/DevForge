...
import { validateJson, jsonToZod } from '@/utils/parsers/json'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'

export function JsonToZod() {
  const tool = getToolById('json-to-zod')!
  ...
  const processJson = useCallback(
    (value: string, name: string = schemaName, source: 'user' | 'example' = 'user') => {
      ...
      try {
        ...
        const result = jsonToZod(parsed, name)
        setOutput(result)
        setStatus('success')
        setErrorMessage('')
  ...
  const handleSchemaNameChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const name = e.target.value || 'schema'
      setSchemaName(name)
      if (autoRun && input) {
