...
import { validateJson, jsonToMongoose } from '@/utils/parsers/json'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'

export function JsonToMongoose() {
  const tool = getToolById('json-to-mongoose')!
  ...
  const processJson = useCallback(
    (value: string, name: string = schemaName, source: 'user' | 'example' = 'user') => {
      if (!value.trim()) {
        setOutput('')
        setStatus('idle')
        setErrorMessage('')
      ...
      try {
        ...
        const result = jsonToMongoose(parsed, name)
        setOutput(result)
        setStatus('success')
        setErrorMessage('')
    ...
    [schemaName, addToolHistory, tool.id]
  )
  ...
  useEffect(() => {
    ...
    setInput(initial)
    if (autoRun) {
      processJson(initial, schemaName, 'example')
    }
  }, [getToolDraft, tool.id, autoRun, processJson, schemaName])
  ...
  const handleSchemaNameChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const name = e.target.value || 'UserSchema'
      setSchemaName(name)
      if (autoRun && input) {
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <Input
            ...
            className="h-8 w-36 text-sm bg-background border-border"
